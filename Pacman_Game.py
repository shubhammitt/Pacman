...
pacman_image = [
    ["0"], [
        "pacman1.png", "pacman11.png"], [
            "pacman2.png", "pacman22.png"], [
                "pacman3.png", "pacman33.png"], [
                    "pacman4.png", "pacman44.png"]]  # file names for pacman images facing different directions
...
k = 0
# to count the number of coins in the given layout of pacman game and give
# initial position to enemy 2 and 3
for i in layout:
    k += 1
...
welcome = font_large.render("WELCOME TO PACMAN", 1,
                            (139, 10, 80), True)  # to print welcome message
...
text = font.render(
    "SCORE: %d" %
    score, 1, (139, 10, 80), True)  # display initial score=0
flag = 0
while True:
    keys = pygame.key.get_pressed()  # to store the keyboard input
    # to store time elapsed in seconds
    seconds = 150 - pygame.time.get_ticks() // 1000
    ...
    draw_pacman(screen, pacman_position,
                pacman_image[direction][randint(0, 1)])
    ...
    if(pacman_position == enemy1 or pacman_position == enemy2 or pacman_position == enemy3 or pacman_position == enemy4 or seconds <= 0):
        ...
        if(lives != 0 and seconds > 0):
            lives -= 1
            another_life = font_large.render(
                "REVIVING", 1, (255, 25, 0, 222), True)
            screen.blit(another_life, (cols * (cols) /
                                       2, rows * (rows + rows / 2) / 2))
            direction = 1
    ...
    if(valid_move(pacman_position, move_direction)):
        ...
        # coin is eaten so increasing score and removing coin
